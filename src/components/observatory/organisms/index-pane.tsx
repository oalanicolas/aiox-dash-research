...
export function IndexPane({
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden border-r border-[var(--rule)]">
      <div className="border-b border-[var(--rule)] px-5 pb-3 pl-5 pr-12 pt-4">
        <div className="mb-3 flex items-baseline justify-between gap-3.5">
          ...
          >
            ...
            <span
              className="truncate text-[13px] italic normal-case tracking-[0] text-[var(--ink-dim)]"
              style={{ fontFamily: SERIF_FONT }}
            >
              {sourceLabel.toLowerCase()} runs
            </span>
          </span>
          <span
        ...
        <label className="flex cursor-text items-center gap-2.5 border-b border-[var(--ink)] py-1.5">
          ...
          <input
            ...
            className="min-w-0 flex-1 bg-transparent text-[13px] tracking-[0.02em] text-[var(--ink)] outline-none placeholder:text-[var(--ink-dim)] placeholder:italic"
            style={{ fontFamily: MONO_FONT }}
          />
          <span
        ...
        {onToggleOverview && (
          ...
          >
            ...
            <span
              className={cn(
                "text-[12px] italic normal-case tracking-[0]",
      ...
      <LightScrollArea ref={listRef} className="min-h-0 flex-1">
        <div className="relative pb-12">
          {visibleRuns.length === 0 ? (
            <div className="px-5 py-6 text-[16px] italic text-[var(--ink-3)]" style={{ fontFamily: SERIF_FONT }}>
              Nada encontrado.
            </div>
          ) : (
            <ol className="m-0 list-none p-0">
              {groupedRuns.map(([groupKey, items], gIdx) => (
                <li key={`${groupKey}-${gIdx}`} className="contents">
                  {group !== "none" && (
                    <div
                      className="sticky top-0 z-[3] flex items-baseline justify-between border-b border-[var(--rule)] bg-[var(--paper)] px-5 pb-2 pt-4 text-[10px] uppercase tracking-[0.18em] text-[var(--ink-3)] shadow-[0_1px_0_var(--rule-soft)] first:pt-2"
                      style={{ fontFamily: MONO_FONT }}
                    >
